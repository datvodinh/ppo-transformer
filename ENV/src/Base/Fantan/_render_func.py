...
class Sprites:
    def __init__(self) -> None:
        ...
        self.gold = Image.open(IMG_PATH+"poker_chip.png").resize((70, 70)).convert("RGBA")
sprites = Sprites()
...
def get_state_image(state=None):
    ...
    for k in range(1, 4):
        ...
        draw_cards(background, np.full(int(n), 0), s, params.list_coords_0[2][1]+120 if k != 2 else params.list_coords_0[2][1], True, faded)
    ...
    draw_outlined_text(draw, text, params.font32, (1050-bbox[2]//2, 590-bbox[3]//2), (255, 255, 255), 1)

    my_chips = int(state[110])
    text = str(my_chips)
    bbox = draw.textbbox((0, 0), text, params.font32)
    draw_outlined_text(draw, text, params.font32, (1050-bbox[2]//2, 300-bbox[3]//2), (255, 255, 255), 1)

    my_chips = int(state[109])
    text = str(my_chips)
    bbox = draw.textbbox((0, 0), text, params.font32)
    draw_outlined_text(draw, text, params.font32, (435-bbox[2]//2, 425-bbox[3]//2), (255, 255, 255), 1)

    my_chips = int(state[111])
    text = str(my_chips)
    bbox = draw.textbbox((0, 0), text, params.font32)
    draw_outlined_text(draw, text, params.font32, (1670-bbox[2]//2, 425-bbox[3]//2), (255, 255, 255), 1)
