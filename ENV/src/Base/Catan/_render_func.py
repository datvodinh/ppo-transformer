...
from src.Base.Catan import env as _env
from src.Base.Catan.env import __MAX_TURN_IN_ONE_GAME__
...
def check_init_map(state):
    ...
    if (checkInitMap.tile_states != state[0:95]).any():
        return True

    if (checkInitMap.prob_2_port_states != state[114:187]).any():
        return True

    if (checkInitMap.prob_states != state[1048:1219]).any():
        return True
...
def init_map(state):
    ...
    checkInitMap.tile_states = state[0:95].copy()
    checkInitMap.prob_2_port_states = state[114:187].copy()
    checkInitMap.prob_states = state[1048:1219].copy()

    tiles = []
    for i in range(19):
        try:
            tiles.append(np.where(state[5 * i : 5 + 5 * i] == 1)[0][0])
        except:
            tiles.append(5)
    ...
    whr_ = np.where(state[114:133] == 1)[0]
    for i in whr_:
        sprites.background.paste(
            sprites.probs[2],
    ...
    for i in range(3, 13):
        if i != 7:
            whr_ = np.where(state[1048 + 19 * v : 1067 + 19 * v] == 1)[0]
            for k in whr_:
                sprites.background.paste(
                    sprites.probs[i],
                    (
                        params.tile_center_pos[k][0] - 27,
                        params.tile_center_pos[k][1] - 1,
                    ),
                    sprites.probs[i],
                )
    ...
    ports = []
    for i in range(9):
        ports.append(np.where(state[133 + 6 * i : 133 + 6 * (i + 1)] == 1)[0][0])
...
def get_state_image(state=None):
    ...
    #  Vẽ Robber
    rob_pos = np.where(state[95:114] == 1)[0][0]
    bg.paste(
        sprites.robber,
        (
    ...
    for i in range(3):
        num_knight = str(int(state[574 + 185 * i]))
        bbox = draw.textbbox((0, 0), num_knight, params.font24)
        draw_outlined_text(
            draw,
            num_knight,
        ...
        longest_road = str(int(state[575 + 185 * i]))
        bbox = draw.textbbox((0, 0), longest_road, params.font24)
        draw_outlined_text(
            draw,
            longest_road,
    ...
    #  Người chơi chính
    p_ids = np.where(state[1044:1047] == 1)[0]
    if len(p_ids) == 0:
        p_idx = 0
    else:
        p_idx = p_ids[0] + 1
    ...
    if p_idx != 0:
        text = f"SysAgent {p_idx}"
        bbox = draw.textbbox((0, 0), text, params.font28)
        draw_outlined_text(
            draw,
            text,
            params.font28,
            (350 - bbox[2] / 2, 100 + 150 * p_idx),
            (0, 0, 0),
            1,
        )
    ...
    #  Trade offer
    for i in range(5):
        text = str(int(state[1028 + i]))
        draw_outlined_text(
            draw, text, params.font24, (455 + 300 + 50 * i, 732), (0, 0, 0), 1
        )
        text = str(int(state[1033 + i]))
        draw_outlined_text(
            draw, text, params.font24, (455 + 300 + 50 * i, 807), (0, 0, 0), 1
        )

    #  Phản hồi của người chơi phụ
    for i in range(3):
        if state[1039 + 2 * i] == 1:
            bg.paste(sprites.icon_check, (597, 293 + 150 * i), sprites.icon_check)

    #  Tổng xx
    if state[946] == 1:
        xx = 7
    elif (state[1219:1224] == 1).any():
        xx = np.where(state[1219:1224] == 1)[0][0] + 2
    elif (state[1224:1229] == 1).any():
        xx = np.where(state[1224:1229] == 1)[0][0] + 8
    else:
        xx = -1
...
phase_annotations = [
    ...
    "Place a settlement",
    "Place a city",
    "Select resources to give in trade",
    "Select resources to receive in trade",
...
action_annotations = [
    ...
    "Choose SysAgent 1 to rob",
    "Choose SysAgent 2 to rob",
    "Choose SysAgent 3 to rob",
    "Buy a road",
    "Buy a settlement",
    "Buy a city",
    "Buy a development card",
    "Create a trade offer with players",
    ...
    "Choose SysAgent 1 to trade with",
    "Choose SysAgent 2 to trade with",
    "Choose SysAgent 3 to trade with",
    "Stop",
    "Stop",
    "Decline all trades (main player)",
]
...
def get_main_player_state(
    ...
    if env_components.winner == -1:
        while env_components.env[230] < __MAX_TURN_IN_ONE_GAME__:
            p_idx = int(env_components.env[254])
        ...
        if (
            ...
            or env_components.env[230] == __MAX_TURN_IN_ONE_GAME__
        ):
            env_components.env[255] = 1
    else:
        env_components.env[255] = 1

    if env_components.env[255] == 0:
        state = _env.getAgentState(env_components.env)
        win = -1
    else:
