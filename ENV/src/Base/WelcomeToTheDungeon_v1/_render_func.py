...
class Sprites:
    def __init__(self) -> None:
        ...
        _monster = ["goblin","skeleton","orc","vampire","golem","lich","demon","dragon"]
        self.cards = []
        for _mon in _monster:
            self.cards.append(Image.open(IMG_PATH+f"monster_{_mon}.png").convert("RGBA").resize(CARD_SIZE))
        ...
        self.players = []
        self.players.append(Image.open(IMG_PATH+f"player.jpg").convert("RGBA").resize(PLAYER_SIZE))
        self.players.append(Image.open(IMG_PATH+f"player.jpg").convert("RGBA").resize(PLAYER_SIZE))
        ...
        self.heroes = []
        self.heroes.append(Image.open(IMG_PATH+f"Warrior.png").convert("RGBA").resize(CARD_SIZE))
        self.heroes.append(Image.open(IMG_PATH+f"Rogue.png").convert("RGBA").resize(CARD_SIZE))

        self.Warrior = []
        for equip in ["knightshield","platearmor","torch","holygrail","dragonspear","vorpalsword"]:
            self.Warrior.append(Image.open(IMG_PATH+f"Warrior_{equip}.png").convert("RGBA").resize(CARD_SIZE))
        
        self.Rogue = []
        for equip in ["buckler","mithrilarmor","ringofpower","invisibilitycloak","healingpotion","vorpaldagger"]:
            self.Rogue.append(Image.open(IMG_PATH+f"Rogue_{equip}.png").convert("RGBA").resize(CARD_SIZE))
        
        self.result = []
        self.result.append(Image.open(IMG_PATH+f"green_tick.png").convert("RGBA").resize(RESULT_SIZE))
        self.result.append(Image.open(IMG_PATH+f"lose_icon.png").convert("RGBA").resize(RESULT_SIZE))
...
class Params:
    def __init__(self) -> None:
        ...
        self.coords_players = [(x_player0,y_player0),(x_player1,y_player1),(x_player2,y_player2),(x_player3,y_player3)]
...
def get_state_image(state=None):
    ...
    for i in range(4):
        ...
        if i % 2 == 0:
            for j in range(numWin):
                background.paste(sprites.result[0], (params.coords_players[i][0]+PLAYER_SIZE[0],params.coords_players[i][1]+ j*RESULT_SIZE[1]))
            for j in range(numLose):
                background.paste(sprites.result[1], (params.coords_players[i][0]+PLAYER_SIZE[0],params.coords_players[i][1]+ (j+2)*RESULT_SIZE[1]))
        else:
            for j in range(numWin):
                background.paste(sprites.result[0], (params.coords_players[i][0]+j*RESULT_SIZE[0],params.coords_players[i][1]+ PLAYER_SIZE[1]))
            for j in range(numLose):
                background.paste(sprites.result[1], (params.coords_players[i][0]+(j+2)*RESULT_SIZE[0],params.coords_players[i][1]+ PLAYER_SIZE[1]))




    if state[39] == 1:
        background.paste(sprites.heroes[0], (round(BG_SIZE[0]*0.15),round(BG_SIZE[1]*0.2)))
        for i in range(6):
            background.paste(sprites.Warrior[i], params.equip[i])

        background.paste(sprites.heroes[1], (round(BG_SIZE[0]*0.55),round(BG_SIZE[1]*0.2)))
        for i in range(6):
            background.paste(sprites.Rogue[i], (params.equip[i][0] + round(BG_SIZE[0]*0.4), params.equip[i][1]))
            
        return background

    if state[38] == 1:
        for i in range(8):
            background.paste(sprites.cards[i], (params.equip[0][0] + (i-1)*CARD_SIZE[0],params.equip[0][1]))
    ...
    if state[14] == 1:
        ...
        background.paste(sprites.heroes[1], (round(BG_SIZE[0]*0.15),round(BG_SIZE[1]*0.2)))
    ...
    if state[37] == 0:
        background.paste(sprites.card_back, (round(BG_SIZE[0]*0.6), round(BG_SIZE[1]*0.35)))
    else:
        _mons = np.where(state[28:36] == 1)[0][0]
        background.paste(sprites.cards[_mons], (round(BG_SIZE[0]*0.6), round(BG_SIZE[1]*0.35)))
    ...
    background.paste(sprites.card_back, (round(BG_SIZE[0]*0.75), round(BG_SIZE[1]*0.35)))                                     
