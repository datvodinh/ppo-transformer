...
from Base.Splendor_v3 import _env
...
action_description = {
    ...
    10: "Taget thẻ thứ 11",
    11: "Taget thẻ thứ 12",
    12: "Taget thẻ đang úp thứ 1",
    13: "Taget thẻ đang úp thứ 2",
    14: "Taget thẻ đang úp thứ 3",
}


class Env_components:
    def __init__(self, env, winner, list_other, lv1, lv2, lv3, list_action) -> None:
        self.env = env
        self.winner = winner
...
def get_env_components():
    ...
    list_action = _env.getValidActions(state)
    env_components = Env_components(env, winner, list_other, lv1, lv2, lv3, list_action)
...
class Sprites:
    def __init__(self) -> None:
        self.font = ImageFont.truetype(
            "ImageFonts/FreeMonoBoldOblique.ttf", int(120 / tl)
        )
        self.font_max = ImageFont.truetype(
            "ImageFonts/FreeMonoBoldOblique.ttf", int(200 / tl)
        )
        self.font2 = ImageFont.truetype(
            "ImageFonts/FreeMonoBoldOblique.ttf", int(60 / tl)
        )
...
def draw_tokens_image(
    ...
    #  for i in range(5):
    #      im.paste(sprites.list_img_token[i], (x, y  ))
    #      ImageDraw.Draw(im).text((x + TOKEN_SIZE[0], y + int(TOKEN_SIZE[1]/3)), f'{list_token_taken[i]}', fill= 'white', font = sprites.font)
    #      x += TOKEN_SIZE[0] + _d2_
...
def get_main_player_state(
    ...
    if env_components.winner == 0:  #  no ended
        while env_components.env[100] <= 400 and env_components.cc < 10000:
            ...
            env_components.list_action = _env.getValidActions(state)
