...
class PPOTransformerModel(nn.Module):
    def __init__(self,config,state_size,action_size):
        ...
        self.fc = self._layer_init(nn.Linear(state_size,config['embed_dim']))

        self.transformer = GatedTransformerXL(config,input_dim=config['embed_dim'])

        self.policy = nn.Sequential([
            nn.ReLU(),
            self._layer_init(nn.Linear(config['embed_dim'],config['hidden_size'])),
            nn.ReLU(),
            self._layer_init(nn.Linear(config['hidden_size'],action_size))
        ])
        ...
        self.value = nn.Sequential([
            ...
            self._layer_init(nn.Linear(config['embed_dim'],config['hidden_size'])),
            nn.ReLU(),
            self._layer_init(nn.Linear(config['hidden_size'],1))
        ])
